...
public class Validator {
	...
	public static int getInt(Scanner scnr, String prompt, int min, int max) {
		...
		do {
			...
			if (number < min) {
				isValid = false;
				System.out.println("The year must be at least " + min);
			} else if (number > max) {
				isValid = false;
				System.out.println("The year must not be after " + max);
			} else {
				isValid = true;
			}
	...
	public static double getDouble(Scanner scnr, String prompt, double min, double max) {
		...
		do {
			...
			if (number < min) {
				isValid = false;
				System.out.println("The price must be at least $" + min);
			} else if (number > max) {
				isValid = false;
	...
	public static int getInt2(Scanner scnr, String prompt) {
		// This approach uses exception handling.
		System.out.print(prompt);
		try {
			int num = scnr.nextInt();
	...
	public static int getInt2(Scanner scnr, String prompt, int min, int max) {
		boolean isValid = false;
		int number;
		do {
			number = getInt(scnr, prompt);
			...
			if (number < min) {
				isValid = false;
				System.out.println("The selection must be at least " + min);
			} else if (number > max) {
				isValid = false;
				System.out.println("The selection must not be over " + max);
			} else {
				isValid = true;
			}
